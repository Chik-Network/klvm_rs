...
use klvmr::serde::{node_from_bytes_backrefs, node_to_bytes};
use klvmr::{ChikDialect, KlvmFlags, run_program_with_counters};
use std::time::Instant;

/// Run a hex-encoded KLVM program and print execution stats
#[derive(Parser, Debug)]
#[command(author, version, about, long_about = None)]
struct Args {
    /// Path to file containing hex-encoded KLVM program
    filename: String,
...
pub fn main() {
    ...
    let program = node_from_bytes_backrefs(&mut a, &program_bytes).expect("invalid KLVM");
    ...
    let mut flags = KlvmFlags::empty();
    for f in &args.flags {
        let matched = KlvmFlags::FLAGS
            .iter()
            .find(|flag| flag.name() == f.as_str());
        match matched {
            Some(flag) => flags |= *flag.value(),
            None => {
                let valid: Vec<&str> = KlvmFlags::FLAGS.iter().map(|f| f.name()).collect();
