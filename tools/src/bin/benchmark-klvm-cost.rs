use chik_bls::{PublicKey, hash_to_g2};
use clap::Parser;
use hex_literal::hex;
...
/// Benchmark KLVM operator costs and validate the cost model. This tool is not
...
fn run_klvm_op(
    ...
    let dialect = ChikDialect::new(
        flags
            | KlvmFlags::ENABLE_SHA256_TREE
            | KlvmFlags::RELAXED_BLS
...
fn warmup_op(op: &OpDef, flags: KlvmFlags) {
    ...
    let atoms: Vec<NodePtr> = op
        ...
        .collect();
    let _ = run_klvm_op(&mut a, op.opcode, &atoms, flags);
}
...
fn measure_op(
    ...
    let atoms: Vec<NodePtr> = op
        ...
        .collect();
    run_klvm_op(&mut a, op.opcode, &atoms, flags)
}
