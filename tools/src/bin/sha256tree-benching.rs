...
/*
This file is for comparing the native sha256tree with the klvm implementation
The costs for the native implementation should be lower as it is not required to
make allocations.

This file also outputs the timings for both the native and klvm versions so we
...
fn time_complete_tree(
    ...
    for i in 1..13 {
        ...
        // klvm
        let start = Instant::now();
        let Reduction(cost_klvm, result_klvm) =
            run_program(a, &dialect, sha_prog, tree, 11_000_000_000).unwrap();
        let duration_klvm = start.elapsed().as_nanos();

        let result_klvm = node_to_bytes(a, result_klvm).expect("node_to_bytes");
        assert_eq!(result_klvm, result_native);

        writeln!(
            output,
            "{leaf_count}\t{duration_native}\t{cost_native}\t{duration_klvm}\t{cost_klvm}"
        )
        ...
        if i == 9 {
            ...
            let ns_per_cost_klvm = duration_klvm as f64 / cost_klvm as f64;
            ...
            println!(
                "KLVM:   {:-7}  {:-7}  {:.4}",
                duration_klvm, cost_klvm, ns_per_cost_klvm
            );
            println!(
                "ratio:    {:.1}%    {:.1}%",
                duration_native as f64 / duration_klvm as f64 * 100.0,
                cost_native as f64 / cost_klvm as f64 * 100.0
            );
        }
    }
}
...
set title \"KLVM implementation vs. native comparison ({variant})\"
...
plot \"{log_file}\" using 1:2 with points title \"native time\" axis x1y1, \\
   ...
   \"{log_file}\" using 1:4 with points title \"KLVM time\" axis x1y1, \\
   \"{log_file}\" using 1:5 with lines title \"KLVM cost\" axis x1y2
"
    )
...
fn main() {
    ...
    let mut flags = KlvmFlags::ENABLE_SHA256_TREE | KlvmFlags::ENABLE_GC;
    if new_cost_model {
