...
use klvmr::chik_dialect::{KlvmFlags, MEMPOOL_MODE};
use klvmr::cost::Cost;
...
use klvmr::serde_2026::{
    SERDE_2026_MAGIC_PREFIX, deserialize_2026, deserialize_2026_body_from_stream, serialize_2026,
};

/// Sane "don't OOM the parser" default. klvm_rs has no consensus opinion;
/// downstream wrappers (e.g. chik_rs) supply their own caps.
...
pub fn run_serialized_chik_program(
    ...
    let flags = KlvmFlags::from_bits_truncate(flags);
    let mut allocator = if flags.contains(KlvmFlags::LIMIT_HEAP) {
...
/// Deserialize KLVM bytes, auto-detecting the format (classic, backrefs, or
...
///
/// This is a Python convenience function — klvm_rs's Rust API doesn't have
...
/// Convert a Python KLVM tree (any object with `.atom` / `.pair` attributes)
...
#[pyfunction]
fn klvm_tree_to_lazy_node(obj: Bound<'_, PyAny>) -> PyResult<LazyNode> {
                ...
                if let Some(bytes) = atom_val {
                    ...
                    if let Some((left, right)) = pair_val {
                        ...
                        return Err(pyo3::exceptions::PyValueError::new_err(
                            "KLVM object has neither .atom nor .pair",
                        ));
                    }
                }
            }
...
fn klvm_rs(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(klvm_tree_to_lazy_node, m)?)?;

    m.add("NO_UNKNOWN_OPS", KlvmFlags::NO_UNKNOWN_OPS.bits())?;
    m.add("LIMIT_HEAP", KlvmFlags::LIMIT_HEAP.bits())?;
    m.add("MEMPOOL_MODE", MEMPOOL_MODE.bits())?;
    m.add("ENABLE_SHA256_TREE", KlvmFlags::ENABLE_SHA256_TREE.bits())?;
    m.add("ENABLE_SECP_OPS", KlvmFlags::ENABLE_SECP_OPS.bits())?;
    m.add("DISABLE_OP", KlvmFlags::DISABLE_OP.bits())?;
    m.add("CANONICAL_INTS", KlvmFlags::CANONICAL_INTS.bits())?;
