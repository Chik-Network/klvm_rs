...
from klvm_rs.klvm_rs import klvm_tree_to_lazy_node, ser_2026
from klvm_rs.program import Program
from klvm_rs.serde import deserialize, serialize
...
class KlvmTreeToLazyNodeTest(unittest.TestCase):
    """Tests for klvm_tree_to_lazy_node."""

    def test_basic_roundtrip(self):
        """Python tree -> klvm_tree_to_lazy_node -> ser -> deser -> assert equal."""
    ...
    def test_content_dedup(self):
        ...
        tree = Program.to((a, b))
        lazy = klvm_tree_to_lazy_node(tree)
        left = lazy.pair[0]
        right = lazy.pair[1]
    ...
    def test_equivalence_with_old_roundtrip(self):
        """New klvm_tree_to_lazy_node path produces same output as old deser_backrefs path."""
        from klvm_rs.klvm_rs import deser_backrefs
        from klvm_rs.ser import sexp_to_bytes
        ...
        for tree in test_cases:
            p = Program.to(tree)
            new_path = ser_2026(klvm_tree_to_lazy_node(p), level=0)
...
class Serde2026RoundTripTest(unittest.TestCase):
    ...
    def check_2026_roundtrip(self, tree, levels=(0,)):
        ...
        p = Program.to(tree)
        for level in levels:
            blob = ser_2026(klvm_tree_to_lazy_node(p), level=level)
            p2 = Program.from_bytes(blob)
    ...
    def test_ser_2026_deser_2026_symmetry(self):
        ...
        from klvm_rs.klvm_rs import deser_2026, ser_2026

        p = Program.to([1, 2, (b"shared", b"shared"), 3])
        blob = ser_2026(klvm_tree_to_lazy_node(p))
