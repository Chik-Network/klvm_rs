...
A block can fit 11billion cost. A significant portion of that cost is typically paid via _size_ (in bytes) and _conditions_ (like `CREATE_COIN` and `AGG_SIG_*`). The worst case scenario is that most of this cost is paid in KLVM execution, i.e. CPU time. The benchmark I've been aiming at is to keep block execution under 10 seconds. That's approximately 1 nanosecond per cost. The `ns/cost` is the main metric I'm using, and somewhere around 0.5 is good.
...
```
cargo run --release --features openssl --bin benchmark-klvm-cost -- --threads 3 --operator <op>
```
...
The KLVM cost model for `modpow` is:
