...
pub fn op_if(a: &mut Allocator, input: NodePtr, _max_cost: Cost, flags: KlvmFlags) -> Response {
...
pub fn op_cons(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: KlvmFlags) -> Response {
    let [n1, n2] = get_args::<2>(a, input, "c")?;
    let r = a.new_pair(n1, n2)?;
    Ok(Reduction(CONS_COST, r))
}

pub fn op_first(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: KlvmFlags) -> Response {
    let [n] = get_args::<1>(a, input, "f")?;
    Ok(Reduction(FIRST_COST, first(a, n)?))
}

pub fn op_rest(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: KlvmFlags) -> Response {
    let [n] = get_args::<1>(a, input, "r")?;
    Ok(Reduction(REST_COST, rest(a, n)?))
}

pub fn op_listp(a: &mut Allocator, input: NodePtr, _max_cost: Cost, flags: KlvmFlags) -> Response {
...
pub fn op_raise(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: KlvmFlags) -> Response {
...
pub fn op_eq(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: KlvmFlags) -> Response {
