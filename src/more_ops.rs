...
pub fn op_sha256(
    ...
    let (base_cost, cost_per_arg, cost_per_byte) = if flags.contains(KlvmFlags::NEW_COST_MODEL) {
        (
...
// possible leading-zero padding in KLVM-serialized atoms.
pub fn op_add(a: &mut Allocator, mut input: NodePtr, max_cost: Cost, flags: KlvmFlags) -> Response {
    use rand::Rng;

    let new_cost_model = flags.contains(KlvmFlags::NEW_COST_MODEL);
...
pub fn op_multiply(
    ...
    while let Some((arg, rest)) = a.next(input) {
        ...
        if first_iter {
            ...
            if flags.contains(KlvmFlags::LIMITS) && !new_cost_model && l0 > 256 {
        ...
        match a.node(arg) {
            NodeVisitor::Buffer(buf) => {
                ...
                if flags.contains(KlvmFlags::LIMITS) && !new_cost_model && l1 > 256 {
                    return Err(EvalErr::InvalidOpArg(arg, "*".to_string()));
                }
        ...
        l0 = total.limbs();
        if flags.contains(KlvmFlags::LIMITS) && !new_cost_model && l0 > 1024 {
            return Err(EvalErr::InvalidOpArg(arg, "*".to_string()));
        }
    }
...
// int_atom) to account for possible leading-zero padding in KLVM-serialized
// atoms.
pub fn op_div(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: KlvmFlags) -> Response {
    if flags.contains(KlvmFlags::MALACHITE) {
    ...
    if flags.contains(KlvmFlags::LIMITS)
        && !flags.contains(KlvmFlags::NEW_COST_MODEL)
    ...
    let cost = if flags.contains(KlvmFlags::NEW_COST_MODEL) {
...
// int_atom) to account for possible leading-zero padding in KLVM-serialized
// atoms.
pub fn op_divmod(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: KlvmFlags) -> Response {
    if flags.contains(KlvmFlags::MALACHITE) {
...
// int_atom) to account for possible leading-zero padding in KLVM-serialized
// atoms.
pub fn op_mod(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: KlvmFlags) -> Response {
    if flags.contains(KlvmFlags::MALACHITE) {
...
pub fn op_gr(a: &mut Allocator, input: NodePtr, _max_cost: Cost, flags: KlvmFlags) -> Response {
    let [v0, v1] = get_args::<2>(a, input, ">")?;
    let (base_cost, cost_per_byte) = if flags.contains(KlvmFlags::NEW_COST_MODEL) {
...
pub fn op_substr(a: &mut Allocator, input: NodePtr, _max_cost: Cost, flags: KlvmFlags) -> Response {
    let new_cost_model = flags.contains(KlvmFlags::NEW_COST_MODEL);
...
pub fn op_ash(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: KlvmFlags) -> Response {
...
#[cfg(test)]
fn test_shift(
    op: fn(&mut Allocator, NodePtr, Cost, KlvmFlags) -> Response,
    a: &mut Allocator,
    a1: &[u8],
    a2: &[u8],
) -> Response {
    ...
    op(a, args, 10000000 as Cost, KlvmFlags::empty())
}
...
pub fn op_lsh(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: KlvmFlags) -> Response {
...
pub fn op_logand(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: KlvmFlags) -> Response {
    let v: Number = (-1).into();
...
pub fn op_logior(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: KlvmFlags) -> Response {
    let v: Number = 0.into();
...
pub fn op_logxor(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: KlvmFlags) -> Response {
    let v: Number = 0.into();
...
pub fn op_not(a: &mut Allocator, input: NodePtr, _max_cost: Cost, _flags: KlvmFlags) -> Response {
...
pub fn op_coinid(a: &mut Allocator, input: NodePtr, _max_cost: Cost, flags: KlvmFlags) -> Response {
    let new_cost_model = flags.contains(KlvmFlags::NEW_COST_MODEL);
...
pub fn op_modpow(a: &mut Allocator, input: NodePtr, max_cost: Cost, flags: KlvmFlags) -> Response {
    if flags.contains(KlvmFlags::MALACHITE) {
...
mod tests {
    ...
    fn test_sha256_atom(buf: &[u8]) {
        ...
        let Reduction(actual_cost, result) =
            op_sha256(&mut a, args, cost, KlvmFlags::empty()).unwrap();
    ...
    fn test_large_operand(
        #[case] op: fn(&mut Allocator, NodePtr, Cost, KlvmFlags) -> Response,
        #[case] arg_size: u32,
        #[case] num_args: u32,
        #[case] expect: Option<EvalErr>,
    ) {
        let mut a = Allocator::new();
        check_large_operand(&mut a, op, arg_size, num_args, KlvmFlags::empty(), &expect);
    }
    ...
    fn test_large_operand_with_flags() {
        type Op = fn(&mut Allocator, NodePtr, Cost, KlvmFlags) -> Response;
        #[allow(clippy::type_complexity)]
        let cases: &[(&str, Op, u32, u32, KlvmFlags, Option<EvalErr>)] = &[
            ("div", op_div, 9, 2, KlvmFlags::DISABLE_OP, None),
            ("div", op_div, 9, 2, KlvmFlags::NEW_COST_MODEL, None),
            ("div", op_div, 9, 2, KlvmFlags::MALACHITE, None),
            ("divmod", op_divmod, 9, 2, KlvmFlags::DISABLE_OP, None),
            ("divmod", op_divmod, 9, 2, KlvmFlags::NEW_COST_MODEL, None),
            ("divmod", op_divmod, 9, 2, KlvmFlags::MALACHITE, None),
            ("modulus", op_mod, 9, 2, KlvmFlags::DISABLE_OP, None),
            ("modulus", op_mod, 9, 2, KlvmFlags::MALACHITE, None),
            ("modulus", op_mod, 9, 2, KlvmFlags::NEW_COST_MODEL, None),
            (
                "modpow",
                op_modpow,
                27,
                3,
                KlvmFlags::NEW_COST_MODEL,
    ...
    #[rstest]
    #[case(KlvmFlags::empty())]
    #[case(KlvmFlags::MALACHITE)]
    fn test_modpow_basic(#[case] flags: KlvmFlags) {
        let mut a = Allocator::new();
    ...
    #[rstest]
    #[case(KlvmFlags::empty())]
    #[case(KlvmFlags::LIMITS)]
    #[case(KlvmFlags::MALACHITE)]
    #[case(KlvmFlags::MALACHITE | KlvmFlags::LIMITS)]
    fn test_modpow_at_256_byte_limit(#[case] flags: KlvmFlags) {
        let mut a = Allocator::new();
    ...
    #[case(true, false, false, KlvmFlags::LIMITS, false)]
    #[case(false, true, false, KlvmFlags::LIMITS, false)]
    #[case(false, false, true, KlvmFlags::LIMITS, false)]
    #[case(true, true, true, KlvmFlags::LIMITS, false)]
    #[case(true, false, false, KlvmFlags::MALACHITE | KlvmFlags::LIMITS, false)]
    #[case(false, true, false, KlvmFlags::MALACHITE | KlvmFlags::LIMITS, false)]
    #[case(false, false, true, KlvmFlags::MALACHITE | KlvmFlags::LIMITS, false)]
    #[case(true, true, true, KlvmFlags::MALACHITE | KlvmFlags::LIMITS, false)]
    // without LIMITS: 257-byte operands must be allowed
    #[case(true, false, false, KlvmFlags::empty(), true)]
    #[case(false, true, false, KlvmFlags::empty(), true)]
    #[case(false, false, true, KlvmFlags::empty(), true)]
    #[case(true, true, true, KlvmFlags::empty(), true)]
    #[case(true, false, false, KlvmFlags::MALACHITE, true)]
    #[case(false, true, false, KlvmFlags::MALACHITE, true)]
    #[case(false, false, true, KlvmFlags::MALACHITE, true)]
    #[case(true, true, true, KlvmFlags::MALACHITE, true)]
    fn test_modpow_oversized(
        #[case] big_base: bool,
        #[case] big_exp: bool,
        #[case] big_mod: bool,
        #[case] flags: KlvmFlags,
        #[case] expect_ok: bool,
    ) {
