...
pub fn uint_atom<const SIZE: usize>(
    ...
    match a.node(args) {
        NodeVisitor::Buffer(bytes) => {
            ...
            if flags.contains(KlvmFlags::CANONICAL_INTS) {
...
mod tests {
    ...
    fn test_uint_atom_4_success(#[case] buf: &[u8], #[case] expected: u64) {
        ...
        assert!(uint_atom::<4>(&a, n, "test", KlvmFlags::empty()) == Ok(expected));
    }
    ...
    fn test_uint_atom_4_failure(#[case] buf: &[u8], #[case] expected: &str) {
        ...
        let n = a.new_atom(buf).unwrap();
        assert_eq!(
            uint_atom::<4>(&a, n, "test", KlvmFlags::empty()),
    ...
    fn test_uint_atom_4_non_canonical(#[case] buf: &[u8], #[case] expected: &str) {
        ...
        let n = a.new_atom(buf).unwrap();
        assert_eq!(
            uint_atom::<4>(&a, n, "test", KlvmFlags::CANONICAL_INTS),
            Err(EvalErr::InvalidOpArg(n, expected.to_string()))
        );
    }
    ...
    fn test_uint_atom_4_pair() {
        ...
        assert_eq!(
            uint_atom::<4>(&a, p, "test", KlvmFlags::empty()),
            Err(EvalErr::InvalidOpArg(
                p,
    ...
    fn test_uint_atom_8_success(#[case] buf: &[u8], #[case] expected: u64) {
        ...
        let mut a = Allocator::new();
        let n = a.new_atom(buf).unwrap();
        assert!(uint_atom::<8>(&a, n, "test", KlvmFlags::empty()) == Ok(expected));
    }
    ...
    fn test_uint_atom_8_failure(#[case] buf: &[u8], #[case] fmt_string: &str) {
        ...
        let n = a.new_atom(buf).unwrap();
        assert_eq!(
            uint_atom::<8>(&a, n, "test", KlvmFlags::empty()),
    ...
    fn test_uint_atom_8_pair() {
        ...
        let p = a.new_pair(n, n).unwrap();
        assert_eq!(
            uint_atom::<8>(&a, p, "test", KlvmFlags::empty()),
            Err(EvalErr::InvalidOpArg(
                p,
                "Requires Int Argument: test".to_string(),
            ))
