...
pub fn op_bls_g1_multiply(
    ...
    let mut cost = if flags.contains(KlvmFlags::NEW_COST_MODEL) {
    ...
    let cost_per_byte = if flags.contains(KlvmFlags::NEW_COST_MODEL) {
...
pub fn op_bls_g1_negate(
    ...
    let strict = !flags.contains(KlvmFlags::RELAXED_BLS);
...
pub fn op_bls_map_to_g1(
    ...
    let (mut cost, cost_per_byte, cost_per_dst_byte) = if flags.contains(KlvmFlags::NEW_COST_MODEL)
    {
        (
...
pub fn op_bls_pairing_identity(
    ...
    let (mut cost, cost_per_arg) = if flags.contains(KlvmFlags::NEW_COST_MODEL) {
