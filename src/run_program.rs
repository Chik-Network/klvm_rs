...
impl<'a, D: Dialect> RunProgramContext<'a, D> {
    ...
    fn apply_op(&mut self, current_cost: Cost, max_cost: Cost) -> Result<Cost> {
        ...
        if op_atom == Some(self.dialect.apply_kw()) {
            ...
            if self.dialect.flags().contains(KlvmFlags::LIMIT_SOFTFORK)
            ...
            let guard_cost = if self.dialect.flags().contains(KlvmFlags::NEW_COST_MODEL) {
                NEW_GUARD_COST
            } else {
                GUARD_COST
            };
...
mod tests {
    ...
    const TEST_CASES: &[RunProgramTest] = &[
        ...
        RunProgramTest {
            prg: "(softfork (q . 979))",
            args: "()",
            flags: KlvmFlags::NO_UNKNOWN_OPS,
            result: None,
            cost: 1000,
        ...
        RunProgramTest {
            prg: "(softfork (q . 919) (q . 9) (q x) (q . ()))",
            args: "()",
            flags: KlvmFlags::NEW_COST_MODEL,
            result: Some("()"),
            cost: 1000,
            err: "",
        },
        ...
        RunProgramTest {
            prg: "(softfork (q . 919) (q . 9) (q x) (q . ()))",
            args: "()",
            flags: KlvmFlags::NEW_COST_MODEL.union(KlvmFlags::NO_UNKNOWN_OPS),
            result: None,
            cost: 1000,
            err: "unknown softfork extension",
        },
        ...
        RunProgramTest {
            ...
            args: "()",
            flags: KlvmFlags::ENABLE_KECCAK_OPS_OUTSIDE_GUARD,
            result: Some("()"),
            cost: 994,
            err: "",
        },
        ...
        RunProgramTest {
            ...
            args: "()",
            flags: KlvmFlags::ENABLE_SECP_OPS,
            result: Some("0"),
            cost: 1300061,
            err: "",
        },
        ...
        RunProgramTest {
            prg: "(sha256tree (q . 10))",
            args: "()",
            flags: KlvmFlags::NO_UNKNOWN_OPS.union(KlvmFlags::ENABLE_SHA256_TREE),
    ...
    fn run_test_case(t: &RunProgramTest) {
        ...
        let dialect = ChikDialect::new(t.flags.union(KlvmFlags::ENABLE_GC));
        println!("prg: {}", t.prg);
    ...
    #[case::ext0_cost_above_new_cost_mempool(
        "(softfork (q . 200) (q . 0) (q . (q . 42)) (q . ()))",
        KlvmFlags::NEW_COST_MODEL.union(KlvmFlags::NO_UNKNOWN_OPS), 700, Some(601), "")]
    ...
    fn test_softfork_new_cost_model_grandfathered(
        ...
        let dialect = ChikDialect::new(flags.union(KlvmFlags::ENABLE_GC));
    ...
    #[case::keccak(
        ...
        (1134, 1, KlvmFlags::ENABLE_KECCAK_OPS_OUTSIDE_GUARD),
        ""
    )]
    #[case::keccak(
    ...
    fn test_softfork(
        #[case] prg: &'static str,
        #[case] fields: (u64, u8, KlvmFlags), // cost, enabled, hard_fork_flag
        #[case] err: &'static str,
        #[values(KlvmFlags::empty())] flags: KlvmFlags,
        ...
        let t = RunProgramTest {
            ...
            args: "()",
            flags,
            result: if err.is_empty() && hard_fork_flag == KlvmFlags::empty() {
                Some("()")
            } else {
                None
            },
            cost: cost - 140,
            err: if hard_fork_flag == KlvmFlags::empty() {
                err
            } else if mempool {
                "unimplemented operator"
            } else {
    ...
    #[rstest]
    #[case::at_limit_with_flag(20, KlvmFlags::LIMIT_SOFTFORK, "")]
    #[case::over_limit_with_flag(21, KlvmFlags::LIMIT_SOFTFORK, "softfork stack depth exceeded")]
    #[case::over_limit_without_flag(21, KlvmFlags::empty(), "")]
    ...
    #[case::new_nested_prehf_outer_unknown_inner_mempool(
        "(softfork (q . 1) (q . 0) (q . (softfork (q . 100) (q . 9) (q x) (q . ()))) (q . ()))",
        KlvmFlags::NEW_COST_MODEL.union(KlvmFlags::NO_UNKNOWN_OPS),
        800,
        None,
        "unknown softfork extension"
    )]
    ...
    fn test_counters() {
        ...
        let (counters, result) = run_program_with_counters(
            &mut a,
            &ChikDialect::new(KlvmFlags::ENABLE_GC),
            program,
            args,
            cost,
        );
